...
from tts_utils import speak, is_speaking, play_beep
...
def start_timer_after_tts(seconds: int):
    """
    TTS가 끝난 뒤부터 seconds 만큼 카운트하는 타이머
    """
    def _worker():
        # 1) TTS가 시작될 때까지 잠깐 대기 (최대 1초 정도)
        for _ in range(10):
...
# 안내 멘트 먼저 TTS로 말하고
    if minutes > 0 and seconds > 0:
        msg = f"{minutes}분 {seconds}초 뒤로 타이머를 설정했습니다."
    elif minutes > 0:
        msg = f"{minutes}분 뒤로 타이머를 설정했습니다."
    else:
        msg = f"{seconds}초 뒤로 타이머를 설정했습니다."

    speak(msg)

# TTS가 끝난 다음부터 타이머가 카운트되도록
    start_timer_after_tts(total_seconds)
